  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!res.ok) throw new Error("Failed to load listing data");
        ...
        setPreviewImages(data.images.map((img) => img.image));
      } catch (err) {
        console.error(err);
      }
    }
      ...
      <div className="container-fluid relative px-3">
        <div className="layout-specing">
          <div className="md:flex justify-between items-center mb-6">
            <div>
              <h5 className="text-lg font-semibold">Edit Listing #{id}</h5>
              <ul className="tracking-[0.5px] inline-flex items-center mt-2">
                ...
                <li className="text-[14px] font-bold">Edit Listing</li>
              </ul>
            </div>
          </div>
